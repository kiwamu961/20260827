...
YOLOで物件写真内の物体を検出し、JavaScriptで写真をカテゴリ分類するプロトタイプです。写真をアップロードすると、処理状況を確認した後、浴室・トイレ・リビングなどのカテゴリ別に分類結果を表示します。

## 確認用URL

**[PhotoSortを開く](https://kiwamu961.github.io/20260827/)**

> GitHub Pagesで `main` ブランチの `/docs` フォルダを公開している場合に利用できます。表示できない場合は、リポジトリの Settings → Pages で公開設定を確認してください。
...
`docs/` 配下は、物件写真分類プロトタイプを公開するためのソースフォルダです。現在は、写真アップロード、処理状況、分類結果のカテゴリ表示までを確認できます。
...
AI物件写真分類サービスの設計・検証資料です。プロトタイプは、1.5日で写真のアップロードから分類結果の表示、手動修正までを確認できる範囲に絞っています。

- [企画書](pj-docs/01_proposal/proposal.md) / [企画概要書](pj-docs/01_proposal/overview.md)
- 要件定義書
- キックオフ資料
- [外部設計書](pj-docs/04_external-design/external-design.md) / 内部設計書
...
```text
PhotoSortの要件定義書と外部設計書を確認してください。
物件写真のアップロード、処理状況、分類結果表示の実装案を、既存の命名規約・入力検証・エラーハンドリング方針に合わせて提案してください。
...
```text
コーディング規約を先に読み、要件定義書F-001〜F-010と外部設計書SC-01〜SC-03に基づいて物件写真分類プロトタイプを実装してください。
既存の構成と命名規約を維持し、変更対象を最小限にしてください。
入力検証、エラーハンドリング、単体テスト、PlaywrightのE2Eテストを含めてください。
