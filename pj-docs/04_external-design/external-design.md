...
要件定義書に基づき、利用者から見える画面、画面遷移、入力・出力、分類結果の表示、およびMVPで利用する外部インターフェースを定義する。

MVPの完了条件は、写真のアップロード、YOLOによる検出、JavaScriptによる分類、カテゴリ別表示、手動修正までの一連の動作を確認できることである。課金、認証、外部API連携はMVPの対象外とする。

## 2. MVPの前提

- MVPは自社検証環境で利用し、ログイン・課金・権限管理は実装しない。
- 画像分類は、YOLOの検出結果と信頼度をJavaScriptのルール処理へ渡して行う。
- 正式な信頼度閾値は未確定とし、MVPでは設定値を変更可能な構成にする。
...
MVPでは外部事業者向けの公開APIを提供しない。画面と分類処理の境界を、将来のAPI化を妨げない形式で定義する。
...
5. 誤分類写真を別カテゴリへ手動修正できる。
6. 画面を再表示しても修正後の分類結果を確認できる。
7. 不要な処理結果を破棄できる。
...
| 版数 | 日付 | 変更内容 | 作成者 |
| --- | --- | --- | --- |
| v0.1 | 2026/09/15 | AI物件写真分類サービスのMVP外部設計書を作成 | 渡邉極 |